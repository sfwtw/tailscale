...
type RestartManagerSession interface {
	...
	// that will be monitored by this restart manager session.
...
func NewRestartManagerSession(logf logger.Logf) (RestartManagerSession, error) {
	...
	result.key = windows.UTF16ToString(sessionKeyBuf[:len(sessionKeyBuf)-1])
	return &result, nil
}
...
func (rms *rmSession) Close() error {
	// 0 is a valid value for rms.session, so we cannot do much here other than
	// just calling rmEndSession and returning its error.
	return rmEndSession(rms.session)
}
...
func (rms *rmSession) processList() ([]_RM_PROCESS_INFO, error) {
	...
	for err == windows.ERROR_MORE_DATA {
...
func (up *UniqueProcess) AsRestartableProcess() (*RestartableProcess, error) {
	...
	var tsSessionID uint32
	if err := windows.ProcessIdToSessionId(up.PID, &tsSessionID); err != nil {
		return nil, fmt.Errorf("ProcessIdToSessionId: %w", err)
	}

	logonSessionID, err := TSSessionIDToLogonSessionID(tsSessionID)
	if err != nil {
		return nil, fmt.Errorf("TSSessionIDToLogonSesisionID: %w", err)
	}
	...
	// These token bits are non-fatal, so we'll proceed with best-effort.
	var userSID string
...
func (rp *RestartableProcess) Close() error {
	...
	if err := windows.CloseHandle(rp.handle); err != nil {
		return err
	}
	rp.handle = 0
...
func (rps RestartableProcesses) Terminate(logf logger.Logf, exitCode uint32, timeout time.Duration) error {
	...
	for len(handles) > 0 {
		...
		logf("unexpected state from WaitForMultipleObjects: %d", waitCode)
	}
...
func StartProcessInSession(logf logger.Logf, sessID SessionID, cmdLineInfo CommandLineInfo) error {
	l := logger.WithPrefix(logf, "winutil.StartProcessInSession: ")
	pi, err := startProcessInSessionInternal(l, sessID, cmdLineInfo, 0)
	if err != nil {
		return err
	}
...
func StartProcessInSessionWithHandler(logf logger.Logf, sessID SessionID, cmdLineInfo CommandLineInfo, handler PostCreateProcessHandler) error {
	l := logger.WithPrefix(logf, "winutil.StartProcessInSessionWithHandler: ")
	pi, err := startProcessInSessionInternal(l, sessID, cmdLineInfo, 0)
	if err != nil {
		return err
	}
...
func RunProcessInSession(logf logger.Logf, sessID SessionID, cmdLineInfo CommandLineInfo, timeout time.Duration) (uint32, error) {
	timeoutMillis, err := wingoes.DurationToTimeoutMilliseconds(timeout)
	if err != nil {
		return 1, err
	}

	l := logger.WithPrefix(logf, "winutil.RunProcessInSession: ")
	pi, err := startProcessInSessionInternal(l, sessID, cmdLineInfo, 0)
	if err != nil {
		return 1, err
	}
	...
	l("Waiting on process for %d ms", timeoutMillis)
	...
	l("Process completed, obtaining exit code")
	var exitCode uint32
	if err := windows.GetExitCodeProcess(pi.Process, &exitCode); err != nil {
		return 1, err
	}
	l("Returning exit code %d", exitCode)
	return exitCode, nil
}

func startProcessInSessionInternal(logf logger.Logf, sessID SessionID, cmdLineInfo CommandLineInfo, extraFlags uint32) (*windows.ProcessInformation, error) {
	logf("Validating cmdLineInfo")
	if err := cmdLineInfo.Validate(); err != nil {
		return nil, err
	}

	logf("Obtaining and validating token for TS session %d", sessID.TSSession)
	...
	logf("Calling CreateProcessAsUser: %s", commandLineStr)
